 ...
 * ----------------------------------------
 * Runs via GitHub Actions on a schedule (every 5 minutes).
 * Uses Telegram getUpdates (long polling) to receive commands,
 * then responds with job filtering options via inline keyboards.
 *
 * Commands:
 *   /start  → Welcome + source selection
 *   /jobs   → Source selection (filter jobs)
 *   /digest → Top 10 jobs from last 30 days
 *   /help   → Command list
 */
...
async function telegramApi(method, payload = {}) {
  ...
  if (!data.ok) {
    console.error(`[Bot] Telegram ${method} error:`, JSON.stringify(data))
  }
  return data
}
...
function loadJobs() {
  ...
  console.warn('[Bot] jobs.json not found.')
  return []
}
...
const countryKb = (src, exp) => ({
  inline_keyboard: [
    ...
    [{ text: '⬅️ Back', callback_data: `f:${src}::` }],
  ],
})
...
const SOURCE_LABELS = {
  ...
  microsoft: '💻 Microsoft', generic: '🌐 Official Portals', all: '✨ All Sources',
}
...
async function showExpScreen(chatId, source, messageId) {
  const lbl = SOURCE_LABELS[source] || source
  await editMessage(chatId, messageId,
    `💼 <b>Step 2 of 3 — Experience Level</b>\n\nSource: <code>${esc(lbl)}</code>\nChoose your experience level:`,
...
async function showResults(chatId, source, exp, country, messageId) {
  await editMessage(chatId, messageId, '⏳ <i>Fetching and filtering jobs… please wait.</i>')
  ...
  if (matched.length === 0) {
    text += `<i>No jobs found. Try broadening your criteria!</i>\n`
  } else {
    ...
    if (matched.length > 8) text += `<i>…and ${(matched.length - 8).toLocaleString()} more. Open dashboard for all results!</i>\n\n`
  }
  ...
  console.log(`[Bot] Results: source=${source} exp=${exp} country=${country} → ${matched.length} jobs`)
}
  ...
  if (text.startsWith('/start') || text.startsWith('/jobs')) {
    ...
    const intro = isStart
      ? `👋 <b>Hey ${esc(name)}! Welcome to SDE Jobs Bot 🤖</b>\n\n` +
        ...
        `Let's find your next role! Choose a source:\n`
      : `🔍 Let's find jobs! Choose a source:`
    ...
    let msg2 = `🌅 <b>Jobs Digest — Last 30 Days</b>\nFound <b>${recent.length.toLocaleString()}</b> SDE openings\n\n`
    recent.slice(0, 10).forEach((job, i) => {
      const posted = job.postedAt ? ` · <i>${esc(job.postedAt.split('T')[0])}</i>` : ''
      msg2 += `<b>${i + 1}. ${esc(job.role || 'SDE')}</b> @ <b>${esc(job.company || 'Unknown')}</b>${posted}\n`
      msg2 += `📍 ${esc(job.location || 'N/A')} | 🔗 <a href="${job.directApplyUrl || job.url || '#'}">Apply</a>\n\n`
    })
    if (recent.length > 10) msg2 += `<i>…and ${(recent.length - 10).toLocaleString()} more!</i>`

    const url = getDashboardUrl()
    if (url) msg2 += `\n\n🌐 <a href="${url}">View Full Dashboard</a>`

    await sendMessage(chatId, msg2, { reply_markup: AGAIN_KB })

  } else if (text.startsWith('/help')) {
    await sendMessage(chatId,
      `🤖 <b>SDE Jobs Bot — Commands</b>\n\n` +
      `/start — Welcome message\n/jobs — Browse &amp; filter jobs interactively\n/digest — Top SDE jobs from last 30 days\n/help — Show this message\n\n` +
      `You'll also receive <b>automatic alerts</b> when new jobs are discovered!`
    )
  } else {
    await sendMessage(chatId, `🤖 Use /jobs to search, /digest for recent jobs, or /help for all commands!`)
  }
}
...
async function main() {
  ...
  console.log(`[Bot] Starting. Offset: ${offset}`)

  const END_AT = Date.now() + 55_000   // run for 55 seconds per invocation
    ...
    try {
      ...
      for (const update of result.result) {
        try {
          ...
          console.error(`[Bot] Update ${update.update_id} error:`, err.message)
        }
  ...
  console.log('[Bot] Session done. Offset saved:', offset)
}
